-- Atualizar a função get_correcoes_ah_stats para usar data_fim_correcao
CREATE OR REPLACE FUNCTION public.get_correcoes_ah_stats()
 RETURNS TABLE(professor_correcao text, mes_atual bigint, mes_anterior bigint, ultimos_3_meses bigint, ultimos_6_meses bigint, ultimos_12_meses bigint)
 LANGUAGE plpgsql
AS $function$
BEGIN
  RETURN QUERY
  WITH dados_com_nomes AS (
    ...
    WHERE pa.professor_correcao IS NOT NULL
      AND pa.data_fim_correcao IS NOT NULL
      AND pa.data_fim_correcao >= CURRENT_DATE - INTERVAL '12 months'
  ),
  ...
  stats_por_professor AS (
    SELECT
      ...
      -- Mês atual
      COALESCE(SUM(CASE 
        WHEN EXTRACT(YEAR FROM dn.data_fim_correcao) = EXTRACT(YEAR FROM CURRENT_DATE)
        AND EXTRACT(MONTH FROM dn.data_fim_correcao) = EXTRACT(MONTH FROM CURRENT_DATE)
        THEN dn.exercicios ELSE 0 END), 0) AS mes_atual,
      
      -- Mês anterior
      COALESCE(SUM(CASE 
        WHEN dn.data_fim_correcao >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 month')
        AND dn.data_fim_correcao < DATE_TRUNC('month', CURRENT_DATE)
        THEN dn.exercicios ELSE 0 END), 0) AS mes_anterior,
      
      -- Últimos 3 meses
      COALESCE(SUM(CASE 
        WHEN dn.data_fim_correcao >= CURRENT_DATE - INTERVAL '3 months'
        THEN dn.exercicios ELSE 0 END), 0) AS ultimos_3_meses,
      
      -- Últimos 6 meses
      COALESCE(SUM(CASE 
        WHEN dn.data_fim_correcao >= CURRENT_DATE - INTERVAL '6 months'
        THEN dn.exercicios ELSE 0 END), 0) AS ultimos_6_meses,
      
      -- Últimos 12 meses
      COALESCE(SUM(CASE 
        WHEN dn.data_fim_correcao >= CURRENT_DATE - INTERVAL '12 months'

-- Migration para atualizar as funções RPC do WhatsApp Comercial
-- Adiciona alterar_nome e join com profiles para mensagens

DROP FUNCTION IF EXISTS get_commercial_conversations_by_phone();
DROP FUNCTION IF EXISTS get_commercial_messages_by_phone(text);
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE((SELECT c.alterar_nome FROM public.clients c WHERE RIGHT(REGEXP_REPLACE(c.phone_number, '\D', '', 'g'), 10) = RIGHT(REGEXP_REPLACE(up.tel, '\D', '', 'g'), 10) LIMIT 1), false) as alterar_nome

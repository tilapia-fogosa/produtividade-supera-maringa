-- Atualizar função para incluir data_falta na lista de reposições
CREATE OR REPLACE FUNCTION public.get_lista_completa_reposicoes()
 RETURNS TABLE(reposicao_id uuid, data_reposicao date, data_falta date, aluno_nome text, turma_original_nome text, turma_reposicao_nome text, observacoes text, unit_id uuid, aluno_id uuid, turma_original_id uuid, turma_reposicao_id uuid)
 LANGUAGE plpgsql
AS $function$
BEGIN
  RETURN QUERY

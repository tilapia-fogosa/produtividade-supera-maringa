...
BEGIN
    ...
    FROM client_activities
    WHERE unit_id = ANY(p_unit_ids)
    AND tipo_atividade = 'Contato Efetivo'
    AND created_at >= p_start_date AND created_at <= p_end_date
...
BEGIN
    ...
    metrics AS (
        SELECT 
            ...
            (SELECT COUNT(*) FROM client_activities ca WHERE ca.created_by = u.user_id AND ca.tipo_atividade = 'Contato Efetivo' AND ca.unit_id = ANY(p_unit_ids) AND ca.created_at >= p_start_date AND ca.created_at <= p_end_date) as effective_contacts,
            ...
            (SELECT COUNT(*) FROM client_activities ca WHERE ca.created_by = u.user_id AND ca.tipo_atividade = 'Agendamento' AND ca.scheduled_date > NOW() AND ca.unit_id = ANY(p_unit_ids) AND ca.created_at >= p_start_date AND ca.created_at <= p_end_date) as awaiting_visits,
...
CREATE OR REPLACE FUNCTION get_closer_performance(p_unit_ids uuid[], p_start_date timestamptz, p_end_date timestamptz)
RETURNS TABLE (
    user_name text,
    assigned_visits bigint,
    completed_visits bigint,
    enrollments bigint
) AS $$
BEGIN
    ...
    WITH closer_users AS (
        SELECT DISTINCT created_by as user_id FROM client_activities WHERE tipo_atividade = 'Atendimento' AND unit_id = ANY(p_unit_ids) AND created_at >= p_start_date AND created_at <= p_end_date
        UNION
        SELECT DISTINCT created_by as user_id FROM atividade_pos_venda WHERE unit_id = ANY(p_unit_ids) AND created_at >= p_start_date AND created_at <= p_end_date
    ),
    metrics AS (
        SELECT 
            u.user_id,
            (SELECT COUNT(*) FROM client_activities ca WHERE ca.created_by = u.user_id AND ca.tipo_atividade = 'Atendimento' AND ca.unit_id = ANY(p_unit_ids) AND ca.created_at >= p_start_date AND ca.created_at <= p_end_date) as completed_visits,
            (SELECT COUNT(*) FROM atividade_pos_venda apv WHERE apv.created_by = u.user_id AND apv.unit_id = ANY(p_unit_ids) AND apv.created_at >= p_start_date AND apv.created_at <= p_end_date) as enrollments
        FROM closer_users u
    )
    SELECT 
        COALESCE((SELECT full_name FROM profiles WHERE profiles.id = metrics.user_id), 'Usuário Desconhecido') as user_name,
        metrics.completed_visits as assigned_visits, 
        metrics.completed_visits,
        metrics.enrollments
    FROM metrics
    WHERE metrics.completed_visits > 0 OR metrics.enrollments > 0;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    metrics AS (
        SELECT 
            ...
            (SELECT COUNT(*) FROM client_activities ca JOIN clients c ON ca.client_id = c.id WHERE COALESCE(c.lead_source, 'Outros') = COALESCE(s.source_name, 'Outros') AND ca.tipo_atividade = 'Contato Efetivo' AND ca.unit_id = ANY(p_unit_ids) AND ca.created_at >= p_start_date AND ca.created_at <= p_end_date) as effective_contacts,
...
BEGIN
    ...
    metrics AS (
        SELECT 
            ...
            (SELECT COUNT(*) FROM client_activities ca WHERE ca.unit_id = u.id AND ca.tipo_atividade = 'Contato Efetivo' AND ca.created_at >= p_start_date AND ca.created_at <= p_end_date) as effective_contacts,
            ...
            (SELECT COUNT(*) FROM client_activities ca WHERE ca.unit_id = u.id AND ca.tipo_atividade = 'Agendamento' AND ca.scheduled_date > NOW() AND ca.created_at >= p_start_date AND ca.created_at <= p_end_date) as awaiting_visits,

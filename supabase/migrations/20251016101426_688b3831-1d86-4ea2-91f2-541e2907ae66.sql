-- Atualizar função get_ah_tempo_stats para incluir tempo início -> fim correção
...
BEGIN
  ...
  SELECT
    ...
    -- NOVO: Tempo médio de início -> fim correção (em dias)
    ROUND(
      AVG(
        CASE 
          WHEN data_inicio_correcao IS NOT NULL 

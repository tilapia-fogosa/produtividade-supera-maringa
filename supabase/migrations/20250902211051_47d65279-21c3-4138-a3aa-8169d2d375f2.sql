-- Criar ou atualizar função RPC para buscar alunos com histórico de retenções
...
BEGIN
    ...
    WITH alunos_dados AS (
        ...
        LEFT JOIN (
            SELECT 
                ...
                COUNT(*) FILTER (WHERE status = 'pendente' OR status = 'em_andamento') as alertas_ativos
            FROM alerta_evasao
            GROUP BY aluno_id
    ...
    alunos_com_status AS (
        SELECT 
            *,
            CASE 
                WHEN alertas_ativos >= 3 THEN 'critico'
                WHEN alertas_ativos > 0 THEN 'alerta'
                WHEN total_retencoes > 0 THEN 'retencao'
                ELSE 'normal'

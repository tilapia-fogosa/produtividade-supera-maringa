...
// Função para calcular datas válidas para faltas (incluindo passadas e futuras)
export const calcularDatasValidasFalta = (diaSemana: string): Date[] => {
  const hoje = new Date();
  const datas: Date[] = [];
  ...
  // Calcular datas passadas (últimos 6 meses)
  let dataAtual = new Date(hoje);
  dataAtual.setMonth(dataAtual.getMonth() - 6); // Voltar 6 meses
  
  // Encontrar a primeira data do dia da semana nos últimos 6 meses
  let diasParaPrimeira = (diaAlvo - getDay(dataAtual) + 7) % 7;
  if (diasParaPrimeira === 0 && getDay(dataAtual) !== diaAlvo) {
    diasParaPrimeira = 7;
  }
  
  let primeiraData = addDays(dataAtual, diasParaPrimeira);
  
  // Gerar todas as datas válidas (passadas e futuras) por aproximadamente 1 ano
  let dataCorrente = new Date(primeiraData);
  const dataLimite = new Date(hoje);
  dataLimite.setMonth(dataLimite.getMonth() + 6); // Próximos 6 meses
  
  while (dataCorrente <= dataLimite) {
    datas.push(new Date(dataCorrente));
    dataCorrente = addDays(dataCorrente, 7); // Próxima semana
  }

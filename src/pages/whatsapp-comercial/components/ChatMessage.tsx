/**
 * Balão de mensagem individual - WhatsApp Comercial
 */
...
export function ChatMessage({ message, onReply }: ChatMessageProps) {
  ...
  const renderMedia = () => {
    ...
    switch (type) {
        ...
        return (
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
              <div className="relative cursor-pointer group/img w-32 h-32 mb-1">
                <img src={message.urlMedia} alt="Imagem" className="w-full h-full object-cover rounded-lg" />
            ...
            <DialogContent className="max-w-[90vw] max-h-[90vh] p-2">
              <img src={message.urlMedia} alt="Imagem ampliada" className="max-w-full max-h-[85vh] object-contain mx-auto" />
            </DialogContent>
          </Dialog>
        );
      case 'videomessage':
        return (
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
              <div className="relative cursor-pointer group/vid w-32 h-32 mb-1 bg-muted rounded-lg flex items-center justify-center">
                <video src={message.urlMedia} className="w-full h-full object-cover rounded-lg" preload="metadata" muted />
            ...
            <DialogContent className="max-w-[90vw] max-h-[90vh] p-2">
              <video src={message.urlMedia} controls autoPlay className="max-w-full max-h-[85vh] mx-auto" />
            </DialogContent>
          </Dialog>
        );
      case 'audiomessage':
      case 'ptt':
      case 'audio':
        return <audio src={message.urlMedia} controls className="w-48 h-10 mb-1" preload="metadata" />;
      case 'documentmessage':
        return (
          <a href={message.urlMedia} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-2 bg-muted rounded-lg mb-1 hover:bg-muted/80 transition-colors">
  ...
  const renderQuotedMessage = () => {
    ...
    return (
      <div className={cn("mb-2 rounded-md px-2 py-1.5 border-l-4 text-xs", message.fromMe ? "bg-black/10 border-l-primary/70" : "bg-muted/60 border-l-primary")}>
        ...
        <p className="truncate opacity-80 mt-0.5">{message.quotedMessage.content || '[Mídia]'}</p>
      </div>
    );
  };
  ...
  return (
    <div className={cn("flex mb-2 group items-center gap-1", message.fromMe ? "justify-end" : "justify-start")}>
      {/* Menu de ações */}
      {onReply && (
        <MessageActionMenu message={message} onReply={onReply} fromMe={message.fromMe} />
      )}
      ...
      <div className={cn(
        ...
        {senderName && <p className="text-xs font-semibold text-primary mb-1 truncate">{senderName}</p>}
        {renderQuotedMessage()}
        {renderMedia()}
        {message.content && (
          <p className="text-sm whitespace-pre-wrap break-words">
            {message.tipoMensagem === 'audio' && <span className="font-bold">Áudio transcrito: </span>}
            {message.content}
          </p>
        )}
          ...
          <div className="flex flex-wrap gap-1 mt-1 -mb-1">
            {(() => {
              ...
              return Object.entries(grouped).map(([emoji, reactions]) => (
                <span key={emoji} className="inline-flex items-center gap-0.5 bg-muted/80 rounded-full px-1.5 py-0.5 text-xs border border-border/50" title={reactions.map(r => r.senderName || 'Alguém').join(', ')}>

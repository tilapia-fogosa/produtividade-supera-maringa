...
import { useFuncionarioDevolutiva, PeriodoFiltro } from '@/hooks/use-funcionario-devolutiva';
import {
  Select,
  SelectContent,
  SelectItem,
...
const DevolutivaFuncionario = () => {
  ...
  const { data: funcionario, loading, error } = useFuncionarioDevolutiva(funcionarioId || '', selectedPeriodo);
  const [textoDevolutiva, setTextoDevolutiva] = useState('');
  const [isOpen, setIsOpen] = useState(false);

  // Atualizar texto da devolutiva quando os dados carregarem
  useEffect(() => {
    if (funcionario?.texto_devolutiva) {
      setTextoDevolutiva(funcionario.texto_devolutiva);
    }
  }, [funcionario?.texto_devolutiva]);
  ...
  const formatarTextoInformativo = (texto: string | null, nomeFuncionario: string) => {
    const primeiroNome = getPrimeiroNome(nomeFuncionario);
    return texto 
  ...
  if (loading) {
    return (
      <div className="container mx-auto py-4 px-2 bg-background dark:bg-background">
        <p className="text-center">Carregando dados do funcionário...</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto py-2 px-2 space-y-3 bg-background dark:bg-background min-h-screen print:py-0 print:pt-2">
      ...
      <div className="text-center mb-2 print:mb-1 print:mt-0">
        <h1 className="text-xl font-bold text-azul-500 print:text-lg print:mb-1">
          Desempenho do Funcionário: {funcionario.nome}
        </h1>
      </div>
      ...
      >
        ...
        <CollapsibleContent className="space-y-4">
          <Textarea
            placeholder="Digite aqui sua mensagem para o funcionário..."
      ...
      <div className="bg-white p-2 rounded-lg border border-orange-200 text-center">
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="text-lg font-semibold text-azul-500">
            {funcionario.abaco_total_desafios} Desafios Realizados
          </span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="bg-white rounded-lg border border-orange-200 overflow-hidden">
          ...
          <Table>
            ...
            <TableBody>
              {funcionario.desempenho_ah.length > 0 ? (
                <>
                  {funcionario.desempenho_ah.map((item, index) => (
                    <TableRow key={index}>
                  ...
                  <TableRow className="font-bold bg-gray-50">
                    ...
                    <TableCell>{funcionario.ah_total_exercicios}</TableCell>
                    <TableCell>{funcionario.ah_total_erros}</TableCell>
                    <TableCell>{funcionario.ah_percentual_total.toFixed(1)}%</TableCell>
                  </TableRow>
                </>
              ) : (
                <TableRow>
        ...
        <div className="bg-white rounded-lg border border-orange-200 overflow-hidden">
          ...
          <Table>
            ...
            <TableBody>
              {funcionario.desempenho_abaco.length > 0 ? (
                <>
        ...
        <div className="bg-gray-50 p-4 rounded-lg border border-orange-200">
          {textoDevolutiva ? (
            <p className="whitespace-pre-wrap">
              {formatarTextoInformativo(textoDevolutiva, funcionario.nome)}
            </p>
          ) : (
            <p className="whitespace-pre-wrap">
              {formatarTextoInformativo(funcionario?.texto_geral, funcionario.nome)}
            </p>
          )}
        </div>
      </div>
...
export default DevolutivaFuncionario;

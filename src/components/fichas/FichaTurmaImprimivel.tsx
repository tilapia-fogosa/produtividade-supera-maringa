...
const FichaTurmaImprimivel: React.FC<FichaTurmaImprimivelProps> = ({
  ...
  return <div className="ficha-container">
      {/* Cabeçalho da ficha */}
      ...
      <Table className="ficha-tabela">
        <TableHeader>
          ...
          <TableRow>
            ...
            {[1, 2, 3, 4, 5].map(dia => <TableHead key={`falta-${dia}`} className="ficha-subcoluna-falta borda-faltas">
                {dia}
              </TableHead>)}
            
            {/* Subcolunas para cada semana */}
            {[1, 2, 3, 4, 5].map(semana => <React.Fragment key={`subheader-semana-${semana}`}>
        ...
        <TableBody>
          {alunosPagina.map((aluno, index) => <TableRow key={aluno.id}>
              ...
              {[1, 2, 3, 4, 5].map(dia => <TableCell key={`falta-${aluno.id}-${dia}`} className="ficha-celula-falta borda-faltas">
                  <div className="falta-campo"></div>
                </TableCell>)}
              
              {/* Células para cada semana (5 subcolunas por semana) */}
              {[1, 2, 3, 4, 5].map(semana => <React.Fragment key={`semana-${aluno.id}-${semana}`}>
          ...
          {alunosPagina.length < alunosPorPagina && Array.from({
          length: alunosPorPagina - alunosPagina.length
        }).map((_, index) => <TableRow key={`empty-row-${index}`}>
              <TableCell className="ficha-celula-nome">
              ...
              {/* Células para faltas (5 dias) */}
              {[1, 2, 3, 4, 5].map(dia => <TableCell key={`empty-falta-${index}-${dia}`} className="ficha-celula-falta borda-faltas">
                  <div className="falta-campo"></div>
                </TableCell>)}
              
              {/* Células para cada semana (5 subcolunas por semana) */}
              {[1, 2, 3, 4, 5].map(semana => <React.Fragment key={`empty-semana-${index}-${semana}`}>
                  <TableCell className={`ficha-celula-semana borda-semana-${semana}`}>
                    <div className="semana-campo-valor"></div>
                  </TableCell>
                  <TableCell className={`ficha-celula-semana borda-semana-${semana}`}>

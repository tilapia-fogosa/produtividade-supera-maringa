...
const FichaTurmaImprimivel: React.FC<FichaTurmaImprimivelProps> = ({
  ...
  const totalPaginas = Math.ceil(alunosOrdenados.length / alunosPorPagina) || 1;
  ...
  const renderizarTabela = (tipoTabela: 'principal' | 'reposicao', paginaAtual: number) => {
    const linhasMax = tipoTabela === 'principal' ? alunosPorPagina : alunosReposicaoPorPagina;

    // Calcular alunos para esta página específica
    const alunosPagina = tipoTabela === 'principal'
      ? alunosOrdenados.slice((paginaAtual - 1) * alunosPorPagina, paginaAtual * alunosPorPagina)
      : []; // Tabela de reposição sempre vazia de alunos pré-preenchidos

    const linhasVazias = linhasMax - alunosPagina.length;
          ...
          {Array.from({ length: linhasVazias }).map((_, index) => (
            <TableRow key={`empty-row-${tipoTabela}-${index}`}>
              <TableCell className="ficha-celula-nome">
                <span className="ficha-aluno-numero">{tipoTabela === 'principal' ? alunosPagina.length + index + 1 + (paginaAtual - 1) * alunosPorPagina : ''}</span>
  ...
  return (
    <>
      {Array.from({ length: totalPaginas }).map((_, index) => {
        ...
        return (
          <div key={`pagina-${paginaAtual}`} className="ficha-print-wrapper">
            <div className="ficha-container">
              ...
              {renderizarTabela('principal', paginaAtual)}
              ...
              {renderizarTabela('reposicao', paginaAtual)}

...
export function KanbanCard({ 
  ...
  return (
    <>
      <Card 
        className={`p-3 mb-2 bg-white shadow-sm hover:shadow-md transition-shadow ${resultado ? 'border-l-4 ' + (resultado === 'evadiu' ? 'border-l-red-500' : 'border-l-green-500') : ''} relative cursor-pointer`}
      ...
      >
        ...
        <div className="space-y-2">
            ...
            <>
              ...
              <div className="flex items-center gap-2 text-xs text-gray-600">
                <div className="flex items-center gap-1">
                  <Checkbox 
                    id={`sgs-${id}`} 
                    checked={exclusao_sgs_confirmada} 
                ...
                <div className="flex items-center gap-1">
                  <Checkbox 
                    id={`whatsapp-${id}`} 
                    checked={exclusao_whatsapp_confirmada}
                    onCheckedChange={(checked) => {
          ...
          {/* Observações para Retidos e Evadidos */}
          {(isEvadidoCard || isRetidoCard) && observacoes_adicionais && (
          ...
          {description && !isEvadidoCard && !isRetidoCard && (
            <p className="text-xs text-gray-600 line-clamp-2">{description}</p>
          )}
          
          {retention_date && !data_retencao_confirmada && (

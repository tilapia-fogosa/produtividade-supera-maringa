...
const AlunoProgressoCard: React.FC<AlunoProgressoCardProps> = ({ alunoId }) => {
  ...
  const [textoDevolutiva, setTextoDevolutiva] = useState(progresso?.texto_devolutiva || '');
  ...
  React.useEffect(() => {
    if (progresso) {
      setTextoDevolutiva(progresso.texto_devolutiva || '');
    }
  }, [progresso]);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="w-full border-orange-200">
        <CardHeader className="pb-2">
          <CardTitle className="text-xl font-bold text-azul-500">
        ...
        <CardContent className="space-y-4">
          ...
          />
          <Button 
            onClick={handleSalvarDevolutiva}
            disabled={salvando}
            className="bg-azul-500 hover:bg-azul-600"
          >
            {salvando ? 'Salvando...' : 'Salvar Devolutiva'}
          </Button>
        </CardContent>
      </Card>
    </div>

...
interface UserProfile {
  ...
  email: string | null;
  role: 'consultor' | 'franqueado' | 'gestor_pedagogico' | 'educador' | 'admin' | 'financeiro' | 'administrativo' | 'estagiario' | 'sala' | 'sdr' | null;
  unit_ids: string[];
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchUserProfile = async (userId: string) => {
    try {
      // Buscar perfil básico incluindo professor_id
      const { data: profileData, error: profileError } = await supabase
        .from('profiles')
        .select('id, full_name, email, gcalendar_id, professor_id')
        .eq('id', userId)
        .single();
